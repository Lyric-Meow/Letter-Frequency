...
def write_statistic():
    file_data = pathlib.Path("in_Eng.txt").read_text(encoding="utf-8").casefold()
    letters_statistic = {}
    ...
    for stat_letter, letter_count in counter.most_common():
        ...
        percent_csv_data += (
            f"{stat_letter}\t{str(round(round(100 / count * letter_count, 2), 4) / 100).replace('.', ',')}\n"
        )

        last = round(100 / count * letter_count, 2)

    pathlib.Path("counts_Eng.txt").write_text(count_data, encoding="utf-8")
    pathlib.Path("procent_Eng.txt").write_text(percent_data, encoding="utf-8")
    pathlib.Path("procentForExcel_Eng.txt").write_text(percent_csv_data, encoding="utf-8")
